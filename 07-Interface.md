...
> 坑 2：断言接口时要理解方法集。`var r io.Reader = bytes.NewReader(...); _, ok := r.(io.Closer)` 的 ok 是 false，因为 `*bytes.Reader` 没有 `Close` 方法。换成同时实现 `io.Reader` 和 `io.Closer` 的 `*os.File` 才会成功。断言成功只表示方法集满足，不代表调用 Close 一定符合当前资源的所有权约定。

> 实践：在错误处理里频繁见到 `if e, ok := err.(interface{ Unwrap() error }); ok {...}`，这是 errors.Is/As 的底层机制（详见[第23章 错误处理](23-错误处理.md)）。
...
对 `int | float64` 这样的运算约束，泛型版本避免了业务代码中的 `any` 断言。Go 规范并不保证每个实例都生成一份完全特化机器码；当前编译器会使用 GC shape、dictionary、内联等策略。是否产生间接调用、转换或逃逸要以编译器输出和 benchmark 为准，详见[第8章 现代类型系统与泛型](./08-现代类型系统与泛型.md)。

**(3) 工程实践与常见坑**

> 坑 1：把值放进 `any` 会构造接口值，但不等于每次都堆分配；是否逃逸取决于值大小、生命周期、调用边界和编译器优化。热路径优先具体类型或泛型，并用 `-gcflags=-m=2` 与 `-benchmem` 验证。
...
3. **对编译期已知的类型集合考虑泛型**。它能消除业务层类型断言，但不承诺完全单态化或零间接调用：
...
- 接口断言与接口转换依赖运行时类型信息和 itab；泛型能替代一部分“只为通用算法而装箱”的场景，但具体性能由 shape、dictionary、内联和逃逸共同决定。
