...
> 引言：Go 的并发标记-清除垃圾回收器是 Runtime 中最复杂的子系统之一。从 Go 1.5 的并发 GC、Go 1.8 的混合写屏障，到 Go 1.19 的 GOMEMLIMIT 和 Go 1.24 的 Cleanup API，机制持续演进。本章公共语义基于 Go 1.26；内部结构按对应版本源码理解，不承诺固定暂停时间或堆规模。
...
栈扫描是 GC 的难点之一：goroutine 可能正在执行，Go 通过**栈收缩 + 抢占点（preemption）+ 异步抢占信号**保证扫描的一致性（详见[第12章 Goroutine](./12-Goroutine.md)）。
...
- **栈上对象不参与三色**：栈分配的对象不进堆位图，扫描时直接走 goroutine 栈。逃逸分析（见[第22章 Escape Analysis](./22-Escape-Analysis.md)）让"不会逃逸的对象"绕开 GC，是 GC 的最大盟友。
...
type gcControllerState struct {
    ...
    memoryLimit  atomic.Int64   // = GOMEMLIMIT（字节），默认 math.MaxInt64
...
**如何"停下"所有 goroutine？** 通过**抢占式调度**：Runtime 给每个正在运行的 P 发信号（`sysmon` 检测 + `preemptone`），goroutine 在下一个安全点（函数序言检查 `stackguard`，或被 `preemptM` 信号异步打断）让出 P。详见[第12章 Goroutine](./12-Goroutine.md)。
...
例：上一轮 GC 后存活 100MiB，`GOGC=100` 会得到约 200MiB 的 heap goal。Pacer 会在低于 goal 的动态 trigger 启动下一轮，使并发标记尽量在堆接近 goal 前完成；goal 不是简单的“到点才开始 GC”。
...
`runtime.SetGCPercent(p)` 返回旧值；负值会关闭 GC，0 表示目标堆不留增长空间而不是关闭。运行时改 GOGC 不需要重启，但频繁动态调参会让容量行为难以解释。

> GOGC 控制的是 heap goal 的增长比例，不是绝对堆大小。存活量大时目标堆也会很大；实际 GC 在动态 trigger 处提前开始。GOMEMLIMIT 用于在比例目标之外提供总内存软预算。

#### 3. 工程实践与常见坑

- **GOGC 不是堆上限**：它只控制增长率，不是硬限制。理解这点是调优的基础。
- **批处理任务可能适合较高 GOGC**：前提是内存预算充足，并且 CPU profile 证明 GC 是成本来源。
- **调低 GOGC 不保证降低尾延迟**：它会增加 GC 频率，可能减少单轮工作量，也可能提高 CPU 与 assist 压力，必须按真实负载比较。
- **不要 GOGC=off**：除非你真的接管 GC 时机（罕见，如守护进程几乎不分配）。堆会无界增长直到 OOM。
- **GOGC 与 GOMEMLIMIT 配合**：先保留默认 GOGC 并设置经过容量核算的 GOMEMLIMIT，再根据 CPU、存活堆和尾延迟实测调整。不存在通用的 `GOGC=200~400` 或固定 90% 配方。

### GOMEMLIMIT

#### 1. 是什么

`GOMEMLIMIT`（Go 1.19+）是**软内存上限**，给 Runtime 一个可用总内存预算，让 GC 在逼近上限时主动加压，而不是只按 GOGC 比例规划。

#### 2. 为什么需要 / 底层实现

容器环境下，cgroup 限定了进程总内存，但 Runtime 不会自动把它转换成 GOMEMLIMIT。GOGC 只按存活堆规划目标，无法独立表达进程总内存预算。

GOMEMLIMIT 修正这点：`heapGoal` 被 `memoryLimit` 截断。当逼近 limit 时，pacer 提前触发、加大 Mark Assist，把堆压回去。

Runtime 以 `MemStats.Sys - MemStats.HeapReleased`，或等价的 `/memory/classes/total:bytes - /memory/classes/heap/released:bytes` 估算受管理内存，再由 pacer 结合存活堆、扫描工作量和内存预算计算目标与 trigger。cgo、`syscall.Mmap`、内核代管内存和二进制映射不完全包含在该预算中。
...
- **容器里显式设置 GOMEMLIMIT**：预算要低于容器限制，并按 cgo、mmap、栈、二进制映射、峰值请求和 sidecar 情况留余量。90% 只能是起始假设，必须压测验证。
...
Go 1.24 的 `runtime.AddCleanup` 可在对象不可达后异步释放其包装的外部资源：
...
func newHandle(fd int) *Handle {
    handle := &Handle{fd: fd}
    runtime.AddCleanup(handle, func(fd int) {
        _ = syscall.Close(fd)
    }, fd)
    return handle
}
...
Cleanup 不是确定性析构：不保证在进程退出前运行，多个 Cleanup 可并发且没有顺序保证。文件、锁、事务等资源仍应提供显式 `Close`，Cleanup 只作为遗漏保护。cleanup 或 arg 不能反向持有 ptr，否则对象永远可达；最后一次系统调用后必要时用 `runtime.KeepAlive` 保证对象不会过早进入清理。

Go 1.26 源码包含 `GOEXPERIMENT=greenteagc` 的 Green Tea 标记算法，目标是通过以 span 为单位的工作组织改善局部性。它在当前基线仍是实验开关，不应写成默认 GC 契约，也不应据此承诺固定性能收益。实验结果必须记录 GOEXPERIMENT、Go commit、堆形态和对照数据。
...
- `runtime/metrics` 用 `/cpu/classes/gc/total:cpu-seconds` 与总 CPU 计算 GC 占比，用 `/gc/pauses:seconds` 观察暂停分布；指标名和单位必须来自 `metrics.All()`。
...
- 避免接口装箱：热路径里 `int` 传给 `any` 会逃逸到堆（详见[第22章](./22-Escape-Analysis.md)）。
...
- 内存敏感：按进程真实非 Go 内存和峰值负载，为 `GOMEMLIMIT` 留出经压测验证的安全余量。
