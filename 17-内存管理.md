...
掌握这些数据结构后，回到 [第16章 Runtime 总览](./16-Runtime总览.md) 的全景图就不再有"黑盒"；后续 [第18章 GC](./18-GC.md) 会基于 `allocBits`/`gcmarkBits`/`sweepgen` 展开标记-清扫细节，[第19章 Scheduler](./19-Scheduler.md) 会基于 P/mcache 绑定关系解释抢占与 syscall 让出 P 的内存侧影响。
