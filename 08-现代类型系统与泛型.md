...
_ = Double(Score(21)) // Score 的底层类型是 int
```
...
Go 1.26 允许泛型类型在自己的类型参数列表中参考自身，可以表达 F-bounded 风格的约束：
...
这不是“泛型类型可以无限递归实例化”，而是解除了类型参数列表中的自引用限制。这种 API 理解成本较高，只在确实需要“操作返回同类型”的抽象时使用。
...
largest := cmp.Max(10, 20)
```

这些函数通常用 `S ~[]E` 或 `M ~map[K]V` 保留调用方的命名类型。使用前仍要理解语义，例如 `slices.Delete` 可能修改底层数组，`maps.Clone` 是浅拷贝。
