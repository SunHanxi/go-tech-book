...
> 引言：Go 程序看似"原生可执行"，实际上每个二进制里都内嵌了一个庞大的运行时（runtime）。Runtime 负责 goroutine 调度、内存分配、垃圾回收、栈管理、网络 I/O 与系统调用拦截。本章是 Runtime 部分的"地图"，后续 [第17章 内存管理](./17-内存管理.md)、[第18章 GC](./18-GC.md)、[第19章 调度器](./19-Scheduler.md) 等都会基于本章的脉络展开。
...
更多细节见 [第19章 Scheduler](./19-Scheduler.md)。
...
掌握本章的"地图"后，[第17章 内存管理](./17-内存管理.md) 将深入每一级缓存的数据结构，[第18章 GC](./18-GC.md) 与 [第19章 Scheduler](./19-Scheduler.md) 会各自展开实现细节。
